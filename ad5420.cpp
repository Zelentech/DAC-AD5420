...
uint32_t DAC_AD5420::ad5420_write(uint32_t data)
{
    ...
    // Write highest byte
    result |=  SPI.transfer((uint8_t)(data >> 16)); // result |=  ad5420_spi_wrap((uint8_t)(data >> 16));
    result <<= 8;

    // Write middle byte
    result |=  SPI.transfer((uint8_t)(data >> 8)); // result |=  ad5420_spi_wrap((uint8_t)(data >> 8));
    result <<= 8;

    // Write lowest byte
    result |= SPI.transfer((uint8_t)(data)); // result |= ad5420_spi_wrap((uint8_t)(data));
...
uint16_t DAC_AD5420::ad5420_read_reg(uint8_t reg_name)
{
    ...
    // Now register data would appear in next three bytes
